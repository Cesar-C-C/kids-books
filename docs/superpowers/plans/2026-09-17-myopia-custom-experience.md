...
- 内容与元数据：`books/myopia/book.js`，保留 14 段双语 PAGES 兼容数据。
- 展示：`books/myopia/index.html`、`myopia-experience.js`、`myopia-experience.css`，使用自然滚动与章节锚点。
- 模型：复用 `myopia.js` 的纯函数，不加载共享翻页阅读器。
...
原计划中 content.js、新绘图、独立自定义音频 ID、tests/qa_myopia_custom.cjs 等尚未落实的草案已被上述实现替代，不应当作剩余任务重做。
