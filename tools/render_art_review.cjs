...
(async()=>{
 ...
 for(let i=0;i<manifest.length;i++){
  ...
  // HTML permits bare ampersands in caption text; librsvg requires XML escaping.
  if(svg){const dom=new JSDOM(svg);svg=new dom.window.XMLSerializer().serializeToString(dom.window.document.querySelector('svg'));dom.window.close();}
