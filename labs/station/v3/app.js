...
const angles = {
 modules: [-.25, .22], node: [-.25, .22], solar: [.62, .26], truss: [-1.42, .16],
 arm: [.52, .26], docking: [-2.00, .16], cupola: [-1.30, -.36],
 windows: [-.25, .22], interior: [-.25, .22], radiator: [-1.20, .52]
};
...
const look = new T.Vector3(-.10, .10, 0), target = { yaw, pitch, distance, look: look.clone() };
 ...
 const exhibit=LabExhibit.create('station',T),viewHistory=[];let openingView=null;
 ...
 function home() {remember();closeExhibit(); active = null; detail = null; mode='outside'; playing = false; autoRotate = false; targetExplosion = 0; target.look.copy(look0); target.yaw = -.72; target.pitch = .22; target.distance = overviewDistance; renderLesson(); }
 ...
 function renderLesson() {
  ...
  $('mechanism-controls').hidden=!active||!['solar', 'arm'].includes(active.region);if($('mechanism-controls').hidden)$('mechanism-help').textContent='';$('part-directory').open=false;$('part-zh-name').after($('operation-panel'));$('operation-panel').after($('explore-section'));updateButtons();
 }
