...
let yaw = .34, pitch = .20, distance = 17, overviewDistance = 17, explosion = 0, targetExplosion = 0, reveal = 0, near = 0;
const look = new T.Vector3(-.30, -.10, 0), target = { yaw, pitch, distance, look: look.clone() };
 ...
 function renderLesson() {
  ...
  $('mechanism-controls').hidden=!active||!['doors'].includes(active.region);if($('mechanism-controls').hidden)$('mechanism-help').textContent='';$('part-directory').open=false;$('part-zh-name').after($('operation-panel'));$('operation-panel').after($('explore-section'));updateButtons();
 }
