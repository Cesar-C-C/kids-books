...
const look = new T.Vector3(-.30, -.10, 0), target = { yaw, pitch, distance, look: look.clone() };
 ...
 function renderLesson() {
  ...
  $('mechanism-controls').hidden=!active||!['doors','engine'].includes(active.region)||['doors.step','doors.emergency'].includes(detail)||(active?.region==='engine'&&detail&&detail!=='engine.fan');if($('mechanism-controls').hidden)$('mechanism-help').textContent='';$('part-zh-name').after($('operation-panel'));$('operation-panel').after($('explore-section'));updateButtons();
 }
