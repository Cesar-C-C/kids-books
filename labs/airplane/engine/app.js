(() => {
 'use strict';
 const $=id=>document.getElementById(id),{parts,tasks}=window.ENGINE_CONTENT;
 ...
 function stopSpeech(){utteranceId++;if('speechSynthesis'in window)speechSynthesis.cancel();$('speech-status').textContent='';}
 function say(text){
  ...
  u.onerror=e=>{if(id===utteranceId&&!['canceled','interrupted'].includes(e.error))$('speech-status').textContent='英文语音暂不可用。可以阅读上面的英文，或检查设备的英语语音设置。';};speechSynthesis.speak(u);
 }
 function select(id,record=true){
  ...
  document.querySelectorAll('[data-engine-part]').forEach(b=>b.setAttribute('aria-pressed',b.dataset.enginePart===id));saveView();renderEvidence();
 }
 ...
 function setFlow(value){flow=value;document.querySelectorAll('[data-flow]').forEach(b=>b.setAttribute('aria-pressed',b.dataset.flow===flow));$('flow-description').textContent=shaft?'正在观察转轴连接，气流暂时隐藏。再次点击“突出转轴连接”可返回气流观察。':descriptions[flow];saveView();}
 document.querySelectorAll('[data-flow]').forEach(b=>b.addEventListener('click',()=>setFlow(b.dataset.flow)));
 ...
 $('language').addEventListener('click',()=>{const english=document.body.classList.toggle('english-only');$('language').setAttribute('aria-pressed',!english);$('language').textContent=english?'显示中文':'中英双语';});
 ...
 function renderTask(index){
  ...
  document.querySelectorAll('[data-task]').forEach(b=>b.setAttribute('aria-pressed',b.dataset.task===t.id));$('task-answers').replaceChildren();
 ...
 function init(){
  ...
  function resize(){const w=viewport.clientWidth,h=viewport.clientHeight;renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix();}new ResizeObserver(resize).observe(viewport);resize();
  ...
  let lastEvidence=0;
  function frame(stamp){requestAnimationFrame(frame);const dt=Math.min(.05,(stamp-lastStamp)/1000||0);lastStamp=stamp;if(document.hidden)return;if(playing){time+=dt*(slow?.25:1);if(stamp-lastEvidence>500){recordMotion();lastEvidence=stamp;}}
 ...
 window.engineDiscovery={snapshot:()=>({rendered,selected,time,open,flow,shaft,playing,camera:{yaw,pitch,distance},journal:journal.read()})};
