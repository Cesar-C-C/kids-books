(() => {
 'use strict';
 // Same progressive-reveal controller as the airplane studio, driving the train
 // airframe plus the running gear. One code path for every assembly: a group of
 // three layers (exterior / interior / ghost), driven by a single update().
 ...
 const tips={
  ...
  doors:'拖动动作滑杆，看两扇门向两侧滑开；门边的感应条和小灯是门控系统。',
 ...
 const helps={
  ...
  doors:'拖动动作滑杆，两扇门向两侧滑开；感应条负责确认门口安全。',
  bogies:'拖动动作滑杆，车轮和车轴一起转动；空气弹簧把车体托住并减振。',
  motors:'拖动动作滑杆，看转子转动；蓝色线路是冷却风道，橙色机壳是牵引电机。',
...
const angles={cab:[-1.72,.20],body:[.30,.22],roof:[.36,.52],windows:[.22,.03],seats:[.34,.40],doors:[.30,.03],coupler:[.58,.12],bogies:[1.62,-.04],motors:[1.62,-.06],pantograph:[1.13,.32]};
...
const minRadius={cab:.85,body:2.0,roof:2.0,windows:1.5,seats:1.5,doors:1.6,coupler:.7,bogies:1.35,motors:1.2,pantograph:.95};
 ...
 const BOGIES=[{id:'bogie-front',x:-2.95},{id:'bogie-rear',x:3.05}];
 const MOTORS=[{id:'motor-front',x:-2.95},{id:'motor-rear',x:3.05}];
 const look0=new T.Vector3(-.1,.1,0);
 let renderer,scene,camera,train,track,assemblies=[],active=null,detail=null,mode='auto',playing=false,slow=true,simTime=0,mechanism=0,autoRotate=false;
 let yaw=-.60,pitch=.36,distance=18,overviewDistance=18,explosion=0,targetExplosion=0,reveal=0,near=0;
 const look=new T.Vector3(-.1,.1,0),target={yaw,pitch,distance,look:look.clone()};
 ...
 function home(){active=null;detail=null;mode='auto';playing=false;autoRotate=false;targetExplosion=0;target.look.copy(look0);target.yaw=-.60;target.pitch=.36;target.distance=overviewDistance;renderLesson();}
 ...
 function renderLesson(){
  ...
  $('part-tip').textContent=detail?p.tip:active?tips[active.region]:'拖动列车，从不同方向看看。点一个部位，或把鼠标放在它上面向前滚动。';
 ...
 $('side-view').onclick=()=>{target.pitch=.04;target.yaw=Math.PI/2;autoRotate=false;updateButtons();};$('top-view').onclick=()=>{target.pitch=1.50;target.yaw=0;autoRotate=false;updateButtons();};$('auto-rotate').onclick=()=>{autoRotate=!autoRotate;updateButtons();};
 $('explode-button').onclick=()=>{targetExplosion=targetExplosion?0:1;detail=null;active=null;target.look.copy(look0);target.distance=overviewDistance*(targetExplosion?1.30:1);playing=false;record();renderLesson();};
 ...
 function init(){
  ...
  train=TrainV3.create(T);assemblies=train.assemblies;scene.add(train.root);
  for(const b of BOGIES){const a=RunningGearV3.createBogie(T,{id:b.id,x:b.x});train.root.add(a.group);assemblies.push(a);}
  for(const m of MOTORS){const a=RunningGearV3.createMotor(T,{id:m.id,x:m.x});train.root.add(a.group);assemblies.push(a);}
  const panto=RunningGearV3.createPantograph(T,{x:2.55});train.root.add(panto.group);assemblies.push(panto);
  ...
  track=RunningGearV3.createTrack(T,{from:-6.3,to:5.7});scene.add(track);
  ...
  const ground=new T.Mesh(new T.PlaneGeometry(140,140),new T.ShadowMaterial({opacity:.13}));ground.rotation.x=-Math.PI/2;ground.position.y=-1.640;ground.receiveShadow=true;scene.add(ground);
  const grid=new T.GridHelper(34,34,0xd1dee7,0xe0e8ef);grid.position.y=-1.645;grid.material.transparent=true;grid.material.opacity=.26;scene.add(grid);
  function resize(){const b=$('viewport').getBoundingClientRect();renderer.setSize(b.width,b.height,false);camera.aspect=b.width/b.height;camera.updateProjectionMatrix();
   ...
   overviewDistance=Math.max(11.5,Math.min(21,5.6/(Math.tan(T.MathUtils.degToRad(21))*Math.max(1,camera.aspect)*.8)));
