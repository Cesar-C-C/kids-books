...
const look0 = new T.Vector3(0, 1.2, 0);
 let renderer, scene, camera, rocket, assemblies = [], active = null, detail = null, mode = 'auto', playing = false, slow = true, simTime = 0, mechanism = 0, autoRotate = false;
...
const look = new T.Vector3(0, 1.2, 0), target = { yaw, pitch, distance, look: look.clone() };
 ...
 function home() { active = null; detail = null; mode = 'auto'; playing = false; autoRotate = false; targetExplosion = 0; target.look.copy(look0); target.yaw = -.62; target.pitch = .20; target.distance = overviewDistance; renderLesson(); }
 ...
 function renderLesson() {
  ...
  $('part-tip').textContent = detail ? p.tip : active ? tips[active.region] : '拖动火箭，从不同方向看看。点一个部位，或把鼠标放在它上面向前滚动。';
 ...
 $('book-view').onclick = $('whole-rocket').onclick = $('home-view').onclick = home; $('back-view').onclick = $('back-part').onclick = back;
 ...
 function init() {
  ...
  window.rocketLab = {
   ...
   snapshot: () => ({ generation: 4, changedOpacity: surfaces.filter(s => s.materials.some(m => Math.abs(m.opacity - materialOriginal.get(m).opacity) > 1e-6)).length, modelId: rocket.root.uuid, selected: active?.region || null, assembly: active?.id || null, detail, near, reveal, mode, playing, simulationTime: simTime, level: mechanism, explosion, meshCount: pickables.length, geometry: rocket.counts, visited: journal.read().found, camera: { yaw, pitch, distance, target: look.toArray() }, assemblies: assemblies.map(a => ({ id: a.id, region: a.region, exterior: a.exterior.visible, interior: a.interior.visible, ghost: a.ghost.visible })), renderer: renderer.info.render }),
 ...
 function frame(stamp) {
  ...
  // out instead of filling the frame — otherwise the boosters sit right in front of
  // the engine bells and there is nothing to see.
