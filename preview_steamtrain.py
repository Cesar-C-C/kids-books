"""One-off hotspot preview for steamtrain (matches preview_overlays.py logic)."""
import os, re
...
for key, img_name in key_map.items():
    img_path = os.path.join(ROOT, 'books', BOOK, 'assets', img_name.replace('.webp', '.png'))
    if not os.path.exists(img_path): print(f"  MISSING: {img_path}"); continue
