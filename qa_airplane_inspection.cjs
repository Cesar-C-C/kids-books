...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({channel:'chrome',headless:true,args:['--enable-webgl','--use-angle=swiftshader','--enable-unsafe-swiftshader']});try{
 ...
 const dir=path.join(out,process.env.AIRPLANE_AUDIT_MOTION==='1'?'airplane-motion-audit':'airplane-audit');fs.mkdirSync(dir,{recursive:true});const shots=[],reports=[];
 async function shot(id){await page.locator('#viewport').screenshot({path:path.join(dir,id+'.png')});shots.push(id);}
 const parts=await page.evaluate(()=>PLANE_PARTS),details=await page.evaluate(()=>AIRPLANE_DETAILS);
 for(const p of parts.filter(p=>process.env.AIRPLANE_AUDIT_LEFT_ONLY!=='1'&&(!process.env.AIRPLANE_AUDIT_PARTS||process.env.AIRPLANE_AUDIT_PARTS.split(',').includes(p.id)))){await click('#home-view');await click(`[data-part="${p.id}"]`);await click('#focus-part');await shot(p.id+'-outside');await click('#open-part');await shot(p.id+'-open');
  for(const d of details.filter(d=>d.region===p.id)){await click(`[data-detail="${d.id}"]`);assert.equal(await page.locator('#part-en').textContent(),d.en);await shot(d.id);reports.push(await page.evaluate(()=>airplaneLab.inspectionReport?.()));if(process.env.AIRPLANE_AUDIT_MOTION==='1'&&['wing.flap','wing.slat','wing.spoiler','aileron.hinge','elevator.hinge','rudder.hinge'].includes(d.id)){await page.locator('#mechanism').evaluate(e=>{e.value=75;e.dispatchEvent(new Event('input',{bubbles:true}));});await frame();await shot(d.id+'-motion');await page.locator('#mechanism').evaluate(e=>{e.value=0;e.dispatchEvent(new Event('input',{bubbles:true}));});await frame();}}
 }
 if(!process.env.AIRPLANE_AUDIT_PARTS)for(const [assembly,region]of [['engine-left','engines'],['wing-left','wings'],['tailplane--1','stabilizers'],['aileron--1','ailerons'],['elevator--1','elevators']]){
  await click('#home-view');await page.evaluate(id=>airplaneLab.select(id),assembly);await frame();await click('#open-part');
  for(const d of details.filter(d=>d.region===region)){await click(`[data-detail="${d.id}"]`);await shot('left-'+d.id);reports.push(await page.evaluate(()=>airplaneLab.inspectionReport()));}
 }
 fs.writeFileSync(path.join(dir,'visibility.json'),JSON.stringify(reports,null,2));
 for(const r of reports){assert.ok(r&&r.points>0,r?.detail+' has geometric samples');assert.ok(r.onScreen/r.points>.9,r.detail+' stays framed');assert.ok(r.visible>=4,r.detail+' has visible target geometry: '+JSON.stringify(r.blockers));}
 ...
 assert.deepEqual(errors,[]);console.log('Captured '+shots.length+' airplane part/open/detail views. Visual inspection required.');
