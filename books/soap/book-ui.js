...
'use strict';
const AUDIO_VER=1;
const SCENE_IMAGES={"water":"water"};
...
function scenes(parent,ids){let prior='',last;ids.forEach(id=>{const item=story.scenes.find(s=>s.id===id),imageKey=SCENE_IMAGES[item.id]||item.image;if(imageKey===prior){last.querySelector('.scene-copy').append(copy(item,true));return;}const card=el('article','scene'),art=el('div','art'),im=el('img');im.src='images/'+imageKey+'.webp';im.alt=t('故事插画','Story illustration');im.loading='lazy';im.onerror=()=>{im.hidden=true;art.append(el('p','pending-art',t('插画正在制作','Illustration in progress')));};art.append(im);const text=el('div','scene-copy');text.append(copy(item));card.append(art,text);parent.append(card);last=card;prior=imageKey;});}
