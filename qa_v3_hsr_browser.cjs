...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({...(process.env.LAB_BROWSER_PATH?{executablePath:process.env.LAB_BROWSER_PATH}:process.env.LAB_BROWSER==='chromium'?{}:{channel:'chrome'}),headless:true,args:['--enable-webgl','--use-angle=swiftshader','--enable-unsafe-swiftshader']});try{
 ...
 await page.screenshot({path:path.join(out,'hsr-v3-seats.png')});
 ...
 for(const id of ['bogies.wheelset','motors.motor','panto.head','coupler.head','cab.desk']){
   ...
   await page.screenshot({path:path.join(out,'hsr-v3-'+id.replace('.','-')+'.png')});
 }
 ...
 await page.screenshot({path:path.join(out,'hsr-v3-doors-open.png')});
 for(const id of ['pantograph','bogies','motors','windows']){
   ...
   await page.locator('#mechanism').evaluate(el=>{el.value=100;el.dispatchEvent(new Event('input',{bubbles:true}));});
   await page.waitForFunction(()=>trainLab.snapshot().level>.99);
   await page.screenshot({path:path.join(out,'hsr-v3-live-'+id+'.png')});
 }
 ...
 assert.ok((await snap()).assemblies.every(a=>a.exterior&&!a.interior&&!a.ghost),'home view must be the whole train, nothing peeled');
 await page.locator('#explode-button').click();await page.waitForFunction(()=>trainLab.snapshot().explosion>.99);
 await page.screenshot({path:path.join(out,'hsr-v3-exploded.png')});
 ...
 for(const [width,height]of[[1024,768],[390,844]]){
   ...
   await page.screenshot({path:path.join(out,'hsr-v3-'+width+'.png')});
 }
 ...
 await page.locator('[data-lab-id="hsr"] .enter-lab').click();await page.waitForFunction(()=>window.trainLab?.snapshot().generation===3);
 await page.locator('.lab-navigation a').last().click();assert.ok(page.url().endsWith('/books/hsr/index.html'));
 await page.locator('.book-lab-link').click();await page.waitForFunction(()=>window.trainLab?.snapshot().generation===3);
 assert.deepEqual(failed,[],'no local asset may 404');
 assert.deepEqual(errors,[],errors.join('\n'));
 console.log(`PASS v3 train browser: shared stage tint and caption layering, 12 assemblies, progressive reveal, 25 english/chinese/principle discoveries, region-gated lever, pause, speech, reassembly, desktop/tablet/mobile, reload, deep link, lab and book doorways.`);
